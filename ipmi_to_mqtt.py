...
LOG = logging.getLogger("ipmi_to_mqtt")
...
def run_ipmitool(command: str) -> str:
    LOG.debug("Running ipmitool command: %s", command)
    proc = subprocess.run(shlex.split(command), check=True, capture_output=True, text=True)
    LOG.debug("ipmitool returned %d bytes", len(proc.stdout))
    return proc.stdout
...
def publish_discovery(client: mqtt.Client, sensor: SensorReading, prefix: str, node_id: str) -> None:
    ...
    LOG.debug("Publishing discovery topic=%s payload=%s", config_topic, payload)
...
def publish_states(client: mqtt.Client, sensors: Iterable[SensorReading], prefix: str, node_id: str) -> None:
    for sensor in sensors:
        ...
        state_topic = f"{prefix}/sensor/{node_id}/{object_id}/state"
        LOG.debug(
            "Publishing state topic=%s sensor=%s value=%s unit=%s",
            state_topic,
            sensor.name,
            sensor.value,
            sensor.unit,
...
def main() -> int:
    ...
    client.loop_start()
    LOG.info(
        "Connected to MQTT broker %s:%d (topic_prefix=%s node_id=%s interval=%ss)",
        args.mqtt_host,
        args.mqtt_port,
        args.topic_prefix,
        args.node_id,
    ...
    while True:
        try:
            ...
            LOG.info("Parsed %d sensor readings", len(sensors))
            for sensor in sensors:
                if sensor.slug not in published_discovery:
                    ...
                    LOG.debug("Discovery published for sensor=%s slug=%s", sensor.name, sensor.slug)
            publish_states(client, sensors, args.topic_prefix, args.node_id)
        except subprocess.CalledProcessError as exc:
            LOG.error("ipmitool command failed (%s): %s", exc.returncode, exc.stderr.strip())
